...
const MarathonDetails = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/marathons/${id}`)
      ...
      .catch(error => console.error('Failed to fetch marathon:', error));
  }, [id]);

  if (!marathon) {
    return <div className="text-center py-20 text-xl font-semibold text-gray-600 dark:text-gray-300">⏳ Loading marathon details...</div>;
  }
    ...
    <div className="max-w-5xl mx-auto p-6 pt-20 bg-gradient-to-br from-white to-blue-50 rounded-xl shadow-xl">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-3 text-gray-700 dark:text-gray-200">
          ...
          <p><strong>🏃 Distance:</strong> {marathon.runningDistance || 'Not specified'}</p>
          <p><strong>🗓️ Starts on:</strong> {marathon.marathonStartDate}</p>
      ...
      <div className="text-center mt-10">
        {isRegistrationOpen() ? (
          ...
          <p className="text-yellow-600 font-medium mt-4">🚫 Registration is currently closed.</p>
        )}
      </div>
    </div>
  );
